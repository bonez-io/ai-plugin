...
- **Rules are editable, not just readable.** A stale or wrong standing rule is a finding too: propose the fix through `bonez:rules` (`update` supersedes in place — never save a duplicate; `delete` expires, nothing is erased). Rules writes are binding guidance mounted into every session in the org — write conservatively and confirm with the user first. Writes need a `read+write` key (a read-only or read+memory key gets a clear `write_scope_required` error, not a silent no-op). Note for Codex specifically: there is no automatic permission prompt gating this call the way the Claude Code leg has — confirming with the user before calling is on you, not the harness.
