...
Search the org's context lake for whatever the user typed after the command.

1. Call the bonez `search` tool with that query. Default to the broad entity set — org knowledge is indexed alongside code:

```json
{"query": "<the argument>", "limit": 10}
```
