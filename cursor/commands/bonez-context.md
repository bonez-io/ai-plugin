...
1. Call the bonez `context` tool. If the user passed an argument after the command, use it as the scope; otherwise omit `scope` entirely for the org-wide mount:

```json
{"scope": "<the argument, if any>"}
```
