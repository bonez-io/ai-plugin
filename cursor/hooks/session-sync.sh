...
# --- bootstrap a fixed entry point for the `stop` hook -------------------------------------
#
# Cursor runs beforeMCPExecution / sessionStart / sessionEnd from the PLUGIN root, but runs
# `stop` from the WORKSPACE root — so `./hooks/...` is not there and the hook dies with exit
# 127. Neither CLAUDE_PLUGIN_ROOT nor PLUGIN_ROOT is exported to a Cursor-plugin hook (both
# tried, both empty, Cursor 3.18.25), so `stop` has no way to name the plugin directory.
#
# The hooks that DO run from the plugin root know where the plugin is. So they leave a shim at
# a path that never moves, and the `stop` entry in hooks.json points at that instead. Rewritten
# on every invocation, so it self-heals if the plugin is reinstalled, upgraded, or moved.
SHIM_DIR="${HOME}/.bonez"
SHIM="${SHIM_DIR}/cursor-hook.sh"
if [ "$EVENT" != "turn-end" ]; then
  WANT="#!/usr/bin/env bash
# Generated by the bonez Cursor plugin. Points Cursor's \`stop\` hook at wherever the plugin
# actually lives — \`stop\` runs from the workspace and cannot find the plugin on its own.
exec \"${HERE}/session-sync.sh\" \"\$@\""
  if [ "$(cat "$SHIM" 2>/dev/null)" != "$WANT" ]; then
    mkdir -p "$SHIM_DIR" 2>/dev/null
    printf '%s\n' "$WANT" > "$SHIM" 2>/dev/null && chmod +x "$SHIM" 2>/dev/null
  fi
fi
